...
function equiposActivos(data){
  // Ahora se muestran todos los espacios de la plantilla,
  // aunque todavía no tengan jugador o equipo.
  return data.equipos;
}
...
function calcularTabla(data){
  ...
  const tabla = activos.map(e => ({
    ...e,
    pts:0, pj:0, g:0, emp:0, p:0, gf:0, gc:0, dg:0, playoffBonus:0
  }));

  function fila(id){ return tabla.find(t=>t.id===id); }
  ...
  const bonusPorRonda = { octavos:4, cuartos:8, semis:14, final:24 };
...
function calcularProbabilidades(tabla){
  ...
  const scores = tabla.map(t => {
    ...
    return {
      ...t,
      scoreProb: Math.max(5, fuerzaEquipo + rendimiento + diferencia + ataque + bonusPartidosPendientes + bonusPlayoffs)
    };
  });
  ...
  return scores
    .map(t => ({...t, prob: Math.round((t.scoreProb / total) * 100)}))
...
function renderPlayoffs(data){
    ...
    partidosActivos.forEach(m=>{
      ...
      html += `
        <div class="playoff-match">
          <div class="team"><img src="${local?.equipoImg || ""}" onerror="this.style.display='none'">${nombreVisible(local?.equipo || "", "Por definir")}</div>
          <div class="score">${score}</div>
          <div class="team"><img src="${visita?.equipoImg || ""}" onerror="this.style.display='none'">${nombreVisible(visita?.equipo || "", "Por definir")}</div>
        </div>
      `;
    });
  ...
  if(!hayDatos){
    playoffsBox.innerHTML = `<div class="empty-playoffs">Playoffs activados, pero todavía no agregaste partidos.</div>`;
  }
}
...
function renderPagina(modo){
  ...
  function renderPartidos(index){
    document.querySelectorAll(".jornadas button").forEach((b,i)=>b.classList.toggle("active",i===index));
    ...
    data.jornadas[index].partidos.forEach(m=>{
      ...
      partidosBox.innerHTML += `
        <div class="match">
          <div class="team"><img src="${local?.equipoImg || ""}" onerror="this.style.display='none'">${nombreVisible(local?.equipo || "", "Equipo")}</div>
          <div class="score">${score}</div>
          <div class="team"><img src="${visita?.equipoImg || ""}" onerror="this.style.display='none'">${nombreVisible(visita?.equipo || "", "Equipo")}</div>
        </div>
      `;
    });
  }
  ...
  if(tabla.length === 0){
    tablaBox.innerHTML = `<tr><td colspan="12">Todavía no agregaste participantes.</td></tr>`;
  }
  ...
  tabla.forEach((t,i)=>{
    tablaBox.innerHTML += `
      <tr>
        ...
        <td><div class="player-cell"><img src="${t.jugadorImg || ""}" onerror="this.style.display='none'">${nombreVisible(t.jugador, "Jugador " + (i+1))}</div></td>
        <td><div class="team-cell"><img src="${t.equipoImg || ""}" onerror="this.style.display='none'">${nombreVisible(t.equipo, "Equipo por definir")}</div></td>
  ...
  if(probs.length === 0){
    probBox.innerHTML = `<div class="empty-playoffs">Agrega participantes para calcular probabilidades.</div>`;
  }
  ...
  probs.forEach(t=>{
    probBox.innerHTML += `
      <div class="prob-card">
        <div class="prob-top">
          <span class="prob-name">${nombreVisible(t.jugador, "Jugador")} - ${nombreVisible(t.equipo, "Equipo por definir")}</span>
